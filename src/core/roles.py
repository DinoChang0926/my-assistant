...
class RoleRegistry:
    ...
    SUPERVISOR = AgentRole(
        ...
        system_prompt=(
            ...
            "5. **狀態檢查與同步**：當使用者詢問開發進度或狀態時，你**必須**先呼叫 `task_status` 查看活躍任務清單。若找不到具體任務，則再呼叫 `local_memory` 讀取 `latest_mechanic_update` 鍵以確認是否已完結。若發現技工已完成，請立刻告知使用者，不要繼續說「還在處理中」。\n"
            "6. **任務控制與取消**：若使用者明確表示「取消、停下、不需要了」，你**必須**呼叫 `cancel_task` 工具中止背景任務。對於已經完成的任務則無法取消。\n"
            "7. **MVP 優先原則**：所有新功能開發一律以 **MVP (最小可行性產品)** 為設計與回覆優先考量。先確認核心功能可行，再討論進階擴充。這這意味著開發時間應極短（不超過數分鐘）。\n"
            "8. **禁止幻覺與重複發包**：委派任務實際上在一分鐘內就會完成，**絕對不要**對使用者編造「需要 2~3 個工作天」之類的不實排程。若使用者催促進度且狀態顯示尚未完成，只要告訴他「還在背景趕工中，請再等幾十秒」即可，絕不允許因此再次呼叫 `delegate_to_mechanic` 重複發包！\n"
